...
func updateHostTimeout(log logrus.FieldLogger, db *gorm.DB, clusterId strfmt.UUID, hostId strfmt.UUID, srcStatus string) (*models.Host, error) {
	var host *models.Host
	var err error

	now := strfmt.DateTime(time.Now())
	if host, err = UpdateHost(log, db, clusterId, hostId, srcStatus, "status_updated_at", now); err != nil {
		return nil, errors.Wrapf(
			err,
			"failed to update host status update time %s from cluster %s state from %s",
